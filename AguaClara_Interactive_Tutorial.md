...
@u.wraps(None, [u.m / u.s, u.m, u.m ** 2 / u.s], False)  ###Command+Enter just runs the code, Shift+Enter runs the code and moves the cursor to the next line automatically, and highlighting the code does not allow the code to run.
...
middlecolumn = myArray[:,1]
middlerow = myArray[1,:]

myNewArray = myArray * u.l
```
...
def stokeseinstein(temp, radius, viscosity, diffcoefficient):
  temp = input('Temperature in Kelvin: ')
  tempuse = temp * u.kelvin
  radius = input('Particle radius in meters: ')
  radiususe = radius * u.m
  eta = input('Viscosity of water: ')
  etause = viscosity * u.kilogram / u.s * u.m

  diffcoefficient = ((6 * tempuse) / (m.pi * viscosity * radiususe))
...
list = [0, 50, 100, 150, 200]
array = np.array(list)

plt.plot(xArray, 5 * xArray / np.sqrt(re_pipe(1, array, pc.viscosity_kinematic(293 * u.kelvin))),

plt.title('Reynolds Number versus Temperature')
